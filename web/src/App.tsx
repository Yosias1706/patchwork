...
function InvestigationPanel({ investigation }: { investigation: Investigation }) {
  const hasMatches = investigation.matches.length > 0;
  return <section className="soft-shadow mt-7 overflow-hidden rounded-2xl border border-[#dfe7dd] bg-white"><div className="border-b border-[#e7ece6] px-5 py-5 sm:px-6"><div className="flex flex-wrap items-center gap-2 text-xs font-bold uppercase tracking-[0.14em] text-moss"><Sparkles size={15} /> Investigation result <EvidencePill confidence={investigation.confidence} /></div><p className="mt-3 max-w-4xl text-sm leading-6 text-[#38584b]">{investigation.summary}</p><p className="mt-3 text-xs text-[#789084]">{evidenceExplanation(investigation.confidence)}</p></div>{hasMatches ? <div className="divide-y divide-[#edf0ec]">{investigation.matches.map((match, index) => <article key={`${match.pull_url}-${index}`} className="px-5 py-5 sm:px-6"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="rounded-md bg-[#eaf2e7] px-1.5 py-0.5 text-[10px] font-bold text-moss">PR #{match.pull_number ?? "?"}</span><a href={match.pull_url} target="_blank" rel="noreferrer" className="inline-flex min-w-0 items-center gap-1 text-sm font-semibold text-ink hover:text-moss"><span className="truncate">{cleanTitle(match.title)}</span><ExternalLink size={14} className="shrink-0" /></a></div><p className="mt-2 max-w-3xl text-sm leading-6 text-[#71877c]">{match.excerpt}</p></div></div><div className="mt-4 flex flex-wrap gap-2">{match.changed_files.slice(0, 4).map((file) => <span key={file} className="inline-flex items-center gap-1 rounded-md bg-[#f5f7f4] px-2 py-1 text-[11px] text-[#698075]"><Code2 size={12} />{file}</span>)}{match.test_files.slice(0, 2).map((file) => <span key={file} className="inline-flex items-center gap-1 rounded-md bg-[#edf5e9] px-2 py-1 text-[11px] text-moss"><Check size={12} />Test: {file}</span>)}</div></article>)}</div> : <div className="px-6 py-10 text-center"><CircleAlert className="mx-auto text-[#8ba095]" size={24} /><p className="mt-3 font-semibold">No similar historical fix found</p><p className="mx-auto mt-2 max-w-md text-sm text-[#71877c]">Try including an error message, affected file, or a shorter description of the behavior.</p></div>}</section>;
}
...
function evidenceExplanation(confidence: Investigation["confidence"]) { if (confidence === "high") return "Several parts of your report point to the same historical fix, and it includes test coverage."; if (confidence === "moderate") return "The closest match shares meaningful behavior or code context with your report."; if (confidence === "low") return "This is a possible lead, but verify the code and tests before relying on it."; return "No indexed historical fix was similar enough to recommend."; }
